...
use super::module_imports::NamedImport;
...
pub struct HelperLoaderStore<'a> {
    ...
    loaded_helper: Rc<RefCell<FxHashMap<Atom<'a>, (Atom<'a>, Option<SymbolId>)>>>,
    imports: Rc<RefCell<std::vec::Vec<(Atom<'a>, NamedImport<'a>)>>>,
}
...
impl<'a> HelperLoaderStore<'a> {
    ...
    fn add_default_import(
        ...
        self.imports.borrow_mut().push((
            source,
            NamedImport::new(bound_ident.name.clone(), None, bound_ident.symbol_id),
        ));
        bound_ident
    }
